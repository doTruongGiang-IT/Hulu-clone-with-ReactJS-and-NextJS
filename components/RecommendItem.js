...
function RecommendItem({recommend}) {
    ...
    return (
        <div className="cursor-pointer w-72 h-36 rounded-md mb-10 relative group">
            <Image className="object-none rounded-md" src={`${URL_IMAGE}${recommend.backdrop_path || recommend.poster_path}` || `${BASE_URL}${recommend.poster_path}`} alt="" width={288} height={144} />
            <div className="flex justify-between text-white">
                <p className="truncate">{recommend.title || recommend.original_title}</p>
                <p>{Math.round(recommend.vote_average * 10)}%</p>
            </div>
